...
public class ESSpawnManager2 : MonoBehaviour
{

	//gateway
	[HideInInspector]public ESGateWaySpawnSetup2[] spawnpoints;
	...
	public float distanceapart = 20f, m_DistApartFromPlayer = 50f, m_SpawnAngle = 50f, LineOfSight = 200f,NearestSpawnDistance = 700f;
	...
	List<ESGateWaySpawnSetup2> nearestspawn; 
	//
	private void Awake()
	{
		Spawn();
		spawnpoints = GameObject.FindObjectsOfType<ESGateWaySpawnSetup2>();
	}
	//
	private void Start()
	{
	...
	{ 
		nearestspawn  = new List<ESGateWaySpawnSetup2>();
		for (int i = 0; i < spawnpoints.Length; ++i)
		{
			if (CalculateDistance(player2.position, spawnpoints[i].transform.position) < NearestSpawnDistance * NearestSpawnDistance)
			{
